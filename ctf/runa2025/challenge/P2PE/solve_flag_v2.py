...
if solver.check() == sat:
    ...
    print(f"c[0]+c[1]+c[2]+c[3] = {chars_test[0]+chars_test[1]+chars_test[2]+chars_test[3]} (expected {0x17d})")
    print(f"c[4]^c[5]^c[6]^c[7] = {chars_test[4]^chars_test[5]^chars_test[6]^chars_test[7]} (expected {0x4c})")
    print(f"Even sum = {sum(chars_test[i] for i in range(0, 32, 2))} (expected {0x6bf})")
    print(f"Odd sum = {sum(chars_test[i] for i in range(1, 32, 2))} (expected {0x656})")
    print(f"\nAll constraints verified: {all([
        chars_test[0]+chars_test[1]+chars_test[2]+chars_test[3] == 0x17d,
        chars_test[4]^chars_test[5]^chars_test[6]^chars_test[7] == 0x4c,
        sum(chars_test[i] for i in range(0, 32, 2)) == 0x6bf,
        sum(chars_test[i] for i in range(1, 32, 2)) == 0x656
    ])}")
else:
